      ...
      <b-col col lg="6" md="10" sm="10">
        <b-card class="card-project mb-3">
          ...
          <b-card-text class="my-4">
            ...
            projets présent sur le site.
            <hr />
            <p class="text-left">
              <u><i>Dernier projet ajouté</i></u> :
              <span v-for="(project, index) in allProjects" :key="index">
                <b-card
                  v-if="index === allProjects.length - 1"
                  :title="project.name"
                  :sub-title="formatDate(project.createdAt)"
                  class="mt-3"
                  >{{ project.name }}</b-card
                >
              </span>
            </p>
            <p class="text-left" v-if="lastUpdatedProject">
              <u><i>Dernier projet modifié</i></u> :
              <b-card
                :title="lastUpdatedProject.name"
                :sub-title="formatDate(lastUpdatedProject.updatedAt)"
                class="mt-3"
                >{{ lastUpdatedProject.name }}</b-card
              >
            </p>
          </b-card-text>
      ...
      <b-col col lg="6" md="10" sm="10">
        <b-card class="card-skill mb-3">
          ...
          <b-card-text class="my-4">
            ...
            <hr />
            <p class="text-left">
              <u><i>Dernière compétence ajoutée</i></u> :
              <span v-for="(skill, index) in allSkills" :key="index">
                <b-card
                  v-if="index === allSkills.length - 1"
                  :title="skill.name"
                  :sub-title="formatDate(skill.createdAt)"
                  class="mt-3"
                  >{{ skill.name }}</b-card
                >
              </span>
            </p>
            <p class="text-left" v-if="lastUpdatedSkill">
              <u><i>Dernière compétence modifiée</i></u> :
              <b-card
                :title="lastUpdatedSkill.name"
                :sub-title="formatDate(lastUpdatedSkill.updatedAt)"
                class="mt-3"
                >{{ lastUpdatedSkill.name }}</b-card
              >
            </p>
          </b-card-text>
          <template #footer> </template>
      ...
      <b-col col lg="6" md="10" sm="10">
        <b-card class="card-career mb-3">
          ...
          <b-card-text class="my-4">
            ...
            <hr />
            <p class="text-left">
              <u><i>Dernière étape de carrière ajoutée</i></u> :
              <span
                v-for="(careerStage, index) in allCareerStages"
                :key="index"
              >
                <b-card
                  v-if="index === allCareerStages.length - 1"
                  :title="careerStage.name"
                  :sub-title="formatDate(careerStage.createdAt)"
                  class="mt-3"
                  >{{ careerStage.name }}</b-card
                >
              </span>
            </p>
            <p class="text-left" v-if="lastUpdatedCareerStage">
              <u><i>Dernière étape de carrière modifiée</i></u> :
              <b-card
                :title="lastUpdatedCareerStage.name"
                :sub-title="formatDate(lastUpdatedCareerStage.updatedAt)"
                class="mt-3"
                >{{ lastUpdatedCareerStage.name }}</b-card
              >
            </p>
          </b-card-text>
          <template #footer> </template>
        ...
        <b-card class="card-presentation mb-3">
          ...
          <b-card-text class="my-4" v-if="onePresentation">
            ...
            <div class="d-flex flex-column text-left">
              <p class="text-left" v-if="onePresentation">
                <u><i>Date d'ajout </i></u>:
              </p>
              <p class="m-auto">{{ formatDate(onePresentation.createdAt) }}</p>
              <p class="text-left mt-3" v-if="onePresentation">
                <u><i>Dernière modification </i></u>:
              </p>
...
import CareerList from "@/components/admin/CareerList.vue";
import SkillList from "@/components/admin/SkillList.vue";
import PresentationList from "@/components/admin/PresentationList.vue";
  ...
  computed: {
    ...
    lastUpdatedProject() {
      ...
      this.allProjects.forEach(project => {
        ...
        if (
          new Date(project.updatedAt).getTime() == Math.max(...dateDiffArray)
        ) {
    ...
    lastUpdatedCareerStage() {
      ...
      this.allCareerStages.forEach(careerStage => {
        ...
        if (
          new Date(careerStage.updatedAt).getTime() ==
          Math.max(...dateDiffArray)
        ) {
