        ...
        <b-tab class="mt-3 justify-content-center">
          ...
          <ValidationObserver ref="modifyForm" v-slot="{ handleSubmit }">
            <b-form @submit.prevent="handleSubmit(onModify)">
              ...
              <div class="d-flex justify-content-center">
                <b-button type="submit" variant="success" class="m-3" :disabled="loading">
                  ...
                  <span class="pl-2 pb-2">Récupérer projet {{ modifyForm.id }}</span>
                </b-button>
              </div>
            </b-form>
            <div id="alertModify" class="mt-3">
              <b-alert 
                variant="success"
                v-show="successMessage"
                v-text="successMessage"
            ...
            <b-card-body class="text-left fst-italic">
              ...
              <b-button variant="primary" class="m-2">Modifier</b-button>
...
export default {
  ...
  methods: {
    ...
    onModify() {      
      this.getProject(this.modifyForm.id)
        .then(() => {
          this.successMessage = 'Voici le projet ' + this.modifyForm.id + ' !' 
        ...
        .catch((error) => {   
          if(error.code == "404") {
            this.errorMessage = 'Le projet ' + this.modifyForm.id  + ' n\'existe pas !';
            this.successMessage = '';
            this.loading = false;
    ...
    onDelete() {
      ...
      this.deleteProject(this.modifyForm.id) 
        .then((response) => {
